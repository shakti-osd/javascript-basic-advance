...
function split1(string, delimiter){
    const res = []
    if(delimiter === "") return Array.from(string)
    ...
    const splitStr = (str, i) => {
        ...
        if( index >= 0){
            res.push(str.substring(0, index))
            splitStr(str.substring(index + delimiter.length), index + delimiter.length)
        } else{
            res.push(str)
        }
    }
...
console.log(split1('The quick the fox jumps the lazy dog.', 'the'))
